...
use numeric_utils::{Interner, IntoOwned, static_assert, static_assert_traits};
use numeric_bits::bit_slice::algos::{BitwiseOps, ElementAdd, ElementSub, ElementMul, ElementShl, ElementShr, ElementBitand, ElementBitor, ElementBitxor, ElementNot};
...
impl BigInt {
    ...
    fn val(&self) -> MaybeInline<'_> {
        if self.is_inline() {
            ...
            MaybeInline::Slice(INT_STORE.get(self.0.offset()))
        }
    }
    ...
    where
        V: Borrow<[usize]> + IntoOwned<Box<[usize]>>,
    {
    ...
    where
        V: IntSlice<usize> + Borrow<[usize]> + IntoOwned<Box<[usize]>>,
    {
        ...
        self.with_slice(|vals| {
            vals.iter()
                ...
                .fold(0., |acc, (idx, val)| {
                    let val = val as f64;
                    let idx = idx as i32;
                    val.mul_add(USIZE_MAX.powi(idx), acc)
                })
        })
    }
}
...
impl_op!(mul(self, rhs) => {
    let out = BigInt::with_slices(self, rhs, |this, other| {
        ElementMul::mul_long(this, other)
    });
...
impl_op!(div(self, rhs) => {
    let out = BigInt::with_slices(self, rhs, |this, other| {
        BitwiseOps::div_long(this, other).0
    });
    BigInt::new_slice(out, self.is_negative() != rhs.is_negative())
});
...
impl_op!(rem(self, rhs) => {
    let out = BigInt::with_slices(self, rhs, |this, other| {
        BitwiseOps::div_long(this, other).1
    });
    BigInt::new_slice(out, self.is_negative() != rhs.is_negative())
});
